...
-- 6. Índices para performance (crear solo si no existen)
...
CREATE OR REPLACE VIEW public.migration_status_v1 AS
SELECT
  'accounts → companies' AS migration,
  COUNT(*)::INT AS total_original,
  (SELECT COUNT(*)::INT FROM public.companies WHERE migration_status = 'migrated') AS migrated_count,
  (SELECT COUNT(*)::INT FROM public.companies WHERE migration_status = 'pending') AS pending_count
FROM public.accounts
UNION ALL
SELECT
  'account_departments → departments',
  COUNT(*)::INT,
  (SELECT COUNT(*)::INT FROM public.departments WHERE migrated_from_account_department_id IS NOT NULL),
  0
...
SELECT
  ...
  COUNT(*)::INT,
  (SELECT COUNT(DISTINCT user_id)::INT FROM public.memberships WHERE migrated_from_profile_id IS NOT NULL),
  0
