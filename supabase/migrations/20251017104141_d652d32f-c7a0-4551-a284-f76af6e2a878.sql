...
-- 1. Crear nuevos enums unificados (solo si no existen)
DO $$ BEGIN
...
-- 6. Índices para performance (solo si no existen)
CREATE INDEX IF NOT EXISTS idx_companies_owner ON public.companies(owner_user_id);
CREATE INDEX IF NOT EXISTS idx_companies_migration ON public.companies(migrated_from_account_id) WHERE migrated_from_account_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_departments_company ON public.departments(company_id);
CREATE INDEX IF NOT EXISTS idx_departments_migration ON public.departments(migrated_from_account_department_id) WHERE migrated_from_account_department_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_memberships_company_role ON public.memberships(company_id, role);
CREATE INDEX IF NOT EXISTS idx_memberships_user ON public.memberships(user_id);
CREATE INDEX IF NOT EXISTS idx_memberships_department ON public.memberships(department_id) WHERE department_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_memberships_migration_profile ON public.memberships(migrated_from_profile_id) WHERE migrated_from_profile_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_profiles_v2_email ON public.profiles_v2(email);
...
DROP TRIGGER IF EXISTS update_profiles_v2_updated_at ON public.profiles_v2;
CREATE TRIGGER update_profiles_v2_updated_at
  BEFORE UPDATE ON public.profiles_v2
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column_v2();

-- 8. Vista para verificación de migración (eliminar primero si existe)
DROP VIEW IF EXISTS public.migration_status_v1;

CREATE VIEW public.migration_status_v1 AS
SELECT
  'accounts → companies' AS migration,
  COUNT(*)::BIGINT AS total_original,
  (SELECT COUNT(*)::BIGINT FROM public.companies WHERE migration_status = 'migrated') AS migrated_count,
  (SELECT COUNT(*)::BIGINT FROM public.companies WHERE migration_status = 'pending') AS pending_count
FROM public.accounts
UNION ALL
SELECT
  'account_departments → departments',
  COUNT(*)::BIGINT,
  (SELECT COUNT(*)::BIGINT FROM public.departments WHERE migrated_from_account_department_id IS NOT NULL),
  0::BIGINT
...
SELECT
  ...
  COUNT(*)::BIGINT,
  (SELECT COUNT(DISTINCT user_id)::BIGINT FROM public.memberships WHERE migrated_from_profile_id IS NOT NULL),
  0::BIGINT
