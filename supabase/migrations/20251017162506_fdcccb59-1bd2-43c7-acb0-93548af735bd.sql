-- 1. Actualizar los registros existentes para asignar department_id
UPDATE profiles p
SET department_id = ad.id
FROM account_departments ad
WHERE p.department = ad.name
  AND p.account_id = ad.account_id
  AND p.department_id IS NULL
  AND p.department IS NOT NULL;

-- 2. Modificar el trigger handle_new_user para asignar department_id
